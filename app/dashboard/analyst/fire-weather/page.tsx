...
const RISK_BG: Record<string, string> = {
  ...
  Unknown: 'bg-ash-700/10 text-ash-400 border-ash-700/30',
}

function fwiColor(v: number | null): string {
  if (v === null) return '#737068'
  ...
  return (
    <span className="relative inline-flex">
      <button
        ...
        onClick={() => setOpen(v => !v)}
        className="text-ash-500 hover:text-ash-300 ml-1 transition-colors"
        aria-label="More info"
      >
        <Info className="w-3.5 h-3.5" />
      </button>
      {open && (
        <span className="absolute left-5 top-0 z-50 w-56 bg-ash-800 border border-ash-600 rounded-lg p-2.5 text-xs text-ash-300 shadow-xl leading-relaxed">
          {text}
        </span>
      )}
    </span>
...
function Skeleton({ className }: { className?: string }) {
  return <div className={`animate-pulse bg-ash-800 rounded-lg ${className}`} />
}
...
function FwiTrendTooltip({ active, payload, label }: any) {
  ...
  return (
    <div className="bg-ash-900 border border-ash-700 rounded-lg px-3 py-2 text-xs shadow-lg">
      <p className="text-ash-400 mb-1">{fmtDate(label)}</p>
      {payload.map((p: any) => (
...
function FwiBarTooltip({ active, payload, label }: any) {
  ...
  return (
    <div className="bg-ash-900 border border-ash-700 rounded-lg px-3 py-2.5 text-xs shadow-lg max-w-xs">
      <p className="text-white font-semibold mb-1">{comp?.fullName ?? label}</p>
      <p className="text-ash-400 mb-1.5">{comp?.description}</p>
      <p style={{ color: '#f04a00' }} className="font-mono font-bold">Value: {payload[0]?.value ?? '—'}</p>
      <p className="text-ash-500 mt-1 border-t border-ash-800 pt-1">{comp?.operationalNote}</p>
    </div>
  )
}
  ...
  return (
    <div className="p-8 max-w-7xl mx-auto space-y-8">
      ...
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          <div className="flex items-center gap-2 text-ash-400 text-xs font-medium mb-2 uppercase tracking-wider">
          ...
          <p className="text-ash-400 text-base max-w-2xl">
      ...
      <div className="card p-5">
        <label className="block text-ash-400 text-xs font-medium mb-2 uppercase tracking-wider">
          Location
        </label>
        <div className="flex gap-3">
          <div className="relative flex-1 max-w-md">
            <MapPin className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-ash-500" />
            <input
              type="text"
              value={locationInput}
              ...
              className="w-full bg-ash-800 border border-ash-700 rounded-lg pl-10 pr-3 py-2.5 text-white text-sm focus:outline-none focus:border-ember-500/60 placeholder:text-ash-600"
            />
          </div>
          <button
        ...
        {!geoError && (
          <p className="text-ash-500 text-xs mt-2 flex items-center gap-1">
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
          {/* Temperature */}
          <div className="card p-5">
            <div className="flex items-center gap-2 text-ash-500 text-xs mb-3">
          ...
          <div className="card p-5">
            ...
            <div className={`font-display text-3xl font-bold ${(data?.current.humidity_pct ?? 100) < 15 ? 'text-signal-danger' : 'text-white'}`}>
          ...
          <div className="card p-5">
            ...
            <div className="font-display text-3xl font-bold text-white">
              ...
              <span className="text-lg font-normal text-ash-400 ml-1">mph</span>
            </div>
            {data?.current.wind_dir && (
              <p className="text-ash-400 text-xs mt-1">{data.current.wind_dir}</p>
            )}
          </div>
          ...
          <div className="card p-5">
            ...
            <p className="text-ash-500 text-xs mt-1">{data?.current.fire_risk ?? '—'}</p>
          </div>
        </div>
      )}
      ...
      <div className="card p-6">
        <div className="flex items-center justify-between mb-4">
          <div>
            <h2 className="font-display text-xl font-bold text-white mb-1">FWI Components</h2>
          ...
          <button
            ...
            className="flex items-center gap-1.5 text-xs text-ash-400 hover:text-white border border-ash-700 hover:border-ash-600 px-3 py-1.5 rounded-lg transition-colors"
          >
        ...
        {loading ? (
          ...
          <>
            <div className="flex flex-wrap gap-x-6 gap-y-1 mb-4">
              {FWI_COMPONENTS.map(c => (
                <span key={c.key} className="text-xs text-ash-400 flex items-center gap-1">
                  <strong className="text-ash-200">{c.label}</strong> {c.fullName}
            ...
            {fwiBarData.length > 0 ? (
              <ResponsiveContainer width="100%" height={200}>
                <BarChart data={fwiBarData} layout="vertical" margin={{ left: 0, right: 20, top: 4, bottom: 4 }}>
                  ...
                  <YAxis type="category" dataKey="label" tick={{ fill: '#b3b1aa', fontSize: 12, fontWeight: 600 }} width={46} />
              ...
              <div className="h-32 flex items-center justify-center text-ash-500 text-sm">
        ...
        {/* Legend table */}
        {showLegend && (
          <div className="mt-4 border-t border-ash-800 pt-4 overflow-x-auto">
            <table className="w-full text-xs">
              <thead>
                <tr className="text-ash-500 uppercase tracking-wider">
              ...
              <tbody className="divide-y divide-ash-800/50">
                {FWI_COMPONENTS.map(c => (
                  <tr key={c.key}>
                    <td className="py-2 pr-4 text-white font-semibold">{c.label}</td>
                    <td className="py-2 pr-4 text-signal-warn font-mono">{c.critical}+</td>
                    <td className="py-2 text-ash-400">{c.operationalNote}</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      <div className="card p-6">
        <div className="flex items-center justify-between mb-1">
          <h2 className="font-display text-xl font-bold text-white">Fire Weather Index — 30 Day Trend</h2>
          {data?.fwi_history_30d && data.fwi_history_30d.length > 0 && (
            <button
              ...
              className="flex items-center gap-2 px-3 py-1.5 text-sm bg-ash-700 hover:bg-ash-600 rounded-lg transition-colors text-ash-200"
            >
              <Download className="w-4 h-4" /> Export FWI Data (CSV)
            </button>
          )}
        </div>
        <p className="text-ash-500 text-xs mb-4">ERA5 reanalysis · FWI (solid) and BUI (dashed) overlay</p>
        ...
        <div className="flex flex-wrap gap-3 mb-4">
          {[
            ...
            <span key={b.label} className="text-xs flex items-center gap-1.5 text-ash-400">
        ...
        {loading ? (
          ...
          <ResponsiveContainer width="100%" height={260}>
            <LineChart data={data.fwi_history_30d} margin={{ left: 0, right: 8, top: 8, bottom: 4 }}>
              ...
              <Legend
                wrapperStyle={{ fontSize: '11px', color: '#b3b1aa' }}
          ...
          <div className="h-48 flex items-center justify-center text-ash-500 text-sm">
      ...
      <div className="card p-6">
        <h2 className="font-display text-xl font-bold text-white mb-1">7-Day Fire Weather Forecast</h2>
        <p className="text-ash-500 text-xs mb-4">Open-Meteo numerical weather forecast · Red rows = fire danger index &gt; 60</p>
        ...
        {loading ? (
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-ash-800 text-left">
                  {['Date', 'Fire Danger Index', 'Max Wind', 'Min Humidity', 'Precip', 'Risk Level'].map(h => (
                    <th key={h} className="px-4 py-3 text-ash-500 text-xs font-medium uppercase tracking-wider">
                      {h}
                    </th>
                  ))}
                </tr>
              ...
              <tbody className="divide-y divide-ash-800">
                {data.forecast_7day.map((row, i) => {
                  ...
                  return (
                    <tr key={i} className={`transition-colors ${highRisk ? 'bg-red-900/10 hover:bg-red-900/20' : 'hover:bg-ash-800/30'}`}>
                      <td className="px-4 py-3 text-white font-medium">{fmtDate(row.date)}</td>
                      <td className="px-4 py-3">
                        <span className="font-mono font-bold" style={{ color: fwiColor(row.fire_danger_index) }}>
                          {row.fire_danger_index ?? '—'}
                        </span>
                      </td>
                      <td className="px-4 py-3 text-ash-300 font-mono">
                        {row.wind_max_mph != null ? `${row.wind_max_mph} mph` : '—'}
                      </td>
                      <td className="px-4 py-3">
                        <span className={`font-mono ${(row.humidity_min ?? 100) < 15 ? 'text-signal-danger' : 'text-ash-300'}`}>
                          {row.humidity_min != null ? `${row.humidity_min}%` : '—'}
                        </span>
                      </td>
                      <td className="px-4 py-3 text-ash-400 font-mono">
          ...
          <div className="h-24 flex items-center justify-center text-ash-500 text-sm">
      ...
      <div className="card p-6">
        <h2 className="font-display text-xl font-bold text-white mb-4">Fuel Moisture Interpretation</h2>
        ...
        <div className="grid md:grid-cols-2 gap-6">
          {/* Fuel classes */}
          <div>
            <h3 className="text-ash-200 font-semibold text-sm mb-3">Fuel Time-Lag Classes</h3>
            <div className="space-y-3">
              {[
                ...
                <div key={f.label} className="bg-ash-800/50 rounded-lg p-3">
                  <p className="text-white text-xs font-semibold">{f.label} <span className="text-ash-500 font-normal">({f.example})</span></p>
                  <p className="text-ash-400 text-xs mt-0.5">{f.desc}</p>
                </div>
              ))}
            </div>
          </div>
          ...
          {/* Current trend */}
          <div>
            <h3 className="text-ash-200 font-semibold text-sm mb-3">Current Conditions Assessment</h3>
            <div className="space-y-3">
              <div className="bg-ash-800/50 rounded-lg p-4">
                <p className="text-ash-400 text-xs mb-1">1-hour fuels (FFMC trend)</p>
                <p className="text-white text-sm">
                  ...
                  {fuelTrend === 'drying' && (
                    <span className="text-ash-400"> Surface fuels are drying — fire spread risk increasing.</span>
                  )}
                  {fuelTrend === 'moistening' && (
                    <span className="text-ash-400"> Surface fuels are gaining moisture — reduced spread risk.</span>
                  )}
                </p>
              </div>

              <div className="bg-ash-800/50 rounded-lg p-4">
                <p className="text-ash-400 text-xs mb-1">Deep fuels (DC)</p>
                <p className="text-white text-sm">
                  DC ={' '}
              ...
              <div className="bg-ash-800/50 rounded-lg p-4">
                <p className="text-ash-400 text-xs mb-1">30-day avg FWI</p>
      ...
      {/* Footer attribution */}
      <p className="text-ash-600 text-xs text-center pb-4">
