...
import { ANALYST_AXIS_TICK, ANALYST_CHART_GRID } from '@/lib/analyst-charts'
...
function ratingBadgeClass(rating: string): string {
  ...
  if (rating === 'Medium') return 'text-gray-800 dark:text-gray-300 border-gray-300 dark:border-gray-600 bg-gray-100 dark:bg-gray-800'
  return 'text-gray-500 dark:text-gray-500 border-gray-200 dark:border-gray-700 bg-gray-100 dark:bg-gray-900'
}
...
function Skeleton() {
  return (
    <div className="p-8 max-w-7xl mx-auto animate-pulse">
      ...
      <div className="h-10 w-96 bg-gray-200 dark:bg-gray-800 rounded mb-3" />
      <div className="h-4 w-80 bg-gray-200 dark:bg-gray-800 rounded mb-8" />
      <div className="grid grid-cols-4 gap-4 mb-8">
        {[...Array(4)].map((_, i) => (
          <div key={i} className="card p-5 h-24 bg-gray-200 dark:bg-gray-800" />
        ))}
      </div>
      <div className="card p-6 mb-6 h-80 bg-gray-200 dark:bg-gray-800" />
      <div className="card p-6 h-64 bg-gray-200 dark:bg-gray-800" />
    </div>
  )
}
...
function ScatterTooltip({ active, payload }: any) {
  ...
  return (
    <div className="bg-gray-900 border border-gray-700 rounded-lg px-3 py-2 text-xs shadow-lg max-w-[200px] text-gray-300">
      ...
      <p className="text-gray-300">SVI: {d.svi.toFixed(2)}</p>
      <p className="text-gray-400">EAL: {formatEAL(d.eal)}</p>
      <p className={`mt-1 text-xs font-medium ${isCompound ? 'text-signal-danger' : 'text-gray-500 dark:text-gray-500'}`}>
      ...
      <div className="p-8 max-w-7xl mx-auto text-gray-700 dark:text-gray-300">
        <div className="card p-6 border border-signal-danger/30 bg-signal-danger/5 flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-gray-600 dark:text-gray-400 text-xs mt-1">{error}</p>
          </div>
        </div>
      </div>
    )
    ...
    <div className="p-8 max-w-7xl mx-auto text-gray-700 dark:text-gray-300">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
        {[
          ...
          { value: String(allStates.length), label: 'High-risk states covered', sub: 'CA · AZ · NM · OR · WA · ID · NV · CO · MT', color: 'text-gray-800 dark:text-gray-300' },
      ...
      <div className="card p-6 mb-6">
        <div className="flex flex-wrap items-start gap-4 justify-between mb-4">
          <div>
            <h3 className="font-display text-lg font-bold text-gray-900 dark:text-white mb-1">NRI Score vs. Social Vulnerability Index</h3>
            ...
            {['All', ...allStates].map(s => (
              <button
                ...
                className={`px-2.5 py-1 rounded-lg text-xs font-medium border transition-all ${
                  stateFilter === s
                    ? 'bg-white dark:bg-gray-700 border-gray-300 dark:border-gray-600 text-gray-900 dark:text-white shadow-sm'
                    : 'border-gray-200 dark:border-gray-800 text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white hover:border-gray-300 dark:hover:border-gray-600'
                }`}
        ...
        <div className="flex flex-wrap gap-3 mb-4">
          {allStates.map(s => (
            <span key={s} className="flex items-center gap-1 text-xs text-gray-600 dark:text-gray-400">
        ...
        <ResponsiveContainer width="100%" height={340}>
          <ScatterChart margin={{ top: 8, right: 16, bottom: 8, left: 0 }}>
            ...
            <XAxis
              ...
              label={{ value: 'NRI Wildfire Score', position: 'insideBottom', offset: -4, fill: '#6b7280', fontSize: 11 }}
            />
            <YAxis
              type="number"
              dataKey="svi"
              ...
              tick={ANALYST_AXIS_TICK}
              width={36}
              label={{ value: 'SVI', angle: -90, position: 'insideLeft', fill: '#6b7280', fontSize: 11 }}
            />
        ...
        {/* Quadrant labels */}
        <div className="grid grid-cols-2 gap-3 mt-4 pt-3 border-t border-gray-200 dark:border-gray-800">
          <div className="card p-3 border border-signal-danger/20 bg-signal-danger/5">
            ...
            <p className="text-gray-500 dark:text-gray-500 text-xs">High wildfire NRI AND high social vulnerability — greatest urgency for early warning infrastructure investment.</p>
          </div>
          <div className="card p-3">
            <div className="text-gray-600 dark:text-gray-400 text-xs font-semibold mb-1">Bottom-left quadrant: Lower Risk</div>
            <p className="text-gray-500 dark:text-gray-500 text-xs">Low NRI AND low SVI — communities with both lower physical risk and greater adaptive capacity.</p>
          </div>
        </div>
      </div>
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-gray-200 dark:border-gray-800 text-left bg-gray-100 dark:bg-gray-800">
                <th className="px-5 py-3 text-gray-600 dark:text-gray-400 text-xs font-medium uppercase tracking-wider">County</th>
                <th className="px-5 py-3 text-gray-600 dark:text-gray-400 text-xs font-medium uppercase tracking-wider">State</th>
                <th className="px-5 py-3 text-gray-600 dark:text-gray-400 text-xs font-medium uppercase tracking-wider">NRI Score</th>
                <th className="px-5 py-3 text-gray-600 dark:text-gray-400 text-xs font-medium uppercase tracking-wider">NRI Rating</th>
                <th className="px-5 py-3 text-gray-600 dark:text-gray-400 text-xs font-medium uppercase tracking-wider">SVI</th>
                <th className="px-5 py-3 text-gray-600 dark:text-gray-400 text-xs font-medium uppercase tracking-wider">Expected Annual Loss</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-200 dark:divide-gray-800">
              {compoundCounties.length === 0 ? (
                <tr>
                  <td colSpan={6} className="px-5 py-8 text-center text-gray-500 dark:text-gray-500 text-sm">
                ...
                compoundCounties.map((c, i) => (
                  <tr key={i} className="hover:bg-gray-100 dark:hover:bg-gray-800/40 transition-colors">
                    <td className="px-5 py-3 text-gray-900 dark:text-white font-medium text-sm">{c.county}</td>
                    ...
                    <td className="px-5 py-3 text-gray-800 dark:text-gray-300 text-sm font-mono">{formatEAL(c.eal)}</td>
                  </tr>
                ))
              )}
            </tbody>
      ...
      <div className="card overflow-hidden mb-6">
        <button
          ...
          className="w-full flex items-center justify-between px-5 py-4 text-left hover:bg-gray-100 dark:hover:bg-gray-800/30 transition-colors"
        >
          <div className="flex items-center gap-2">
            <Database className="w-4 h-4 text-gray-600 dark:text-gray-400" />
            <span className="text-gray-900 dark:text-white font-semibold text-sm">Methodology &amp; Data Definitions</span>
          </div>
          {methodOpen
        ...
        {methodOpen && (
          <div className="px-5 pb-5 border-t border-gray-200 dark:border-gray-800 pt-4 space-y-4">
            <div>
              <div className="text-gray-900 dark:text-white font-semibold text-sm mb-1">Expected Annual Loss (EAL)</div>
            ...
            <div>
              <div className="text-gray-900 dark:text-white font-semibold text-sm mb-1">WLDF_RISKR Rating Scale</div>
            ...
            <div>
              <div className="text-gray-900 dark:text-white font-semibold text-sm mb-1">Why Compound Vulnerability Matters</div>
              <p className="text-gray-600 dark:text-gray-400 text-sm leading-relaxed">
            ...
            <div>
              <div className="text-gray-900 dark:text-white font-semibold text-sm mb-1">Data Sources</div>
              <ul className="text-gray-600 dark:text-gray-400 text-sm space-y-0.5 list-disc list-inside">
